...
"""
            ...
            if decision == "buy":
                ...
                if buy_amount > 5000:
                    logger.info(f"Buy Order: {percentage}% of available KRW (Amount: {buy_amount})")
                    # 실제로 매수 주문을 실행하지 않고 로그에만 출력
                    order_executed = True
                else:
                ...
                if sell_amount * current_price > 5000:
                    logger.info(f"Sell Order: {percentage}% of held BTC (Amount: {sell_amount})")
                    # 실제로 매도 주문을 실행하지 않고 로그에만 출력
                    order_executed = True
                else:
